...
theorem Ymd.lt_def' (d₁ d₂ : Ymd) :
  ...
  (d₁.year < d₂.year
   ...
   match lt_trichotomy d₁.year d₂.year with
   | .inl y_lt => simp [y_lt]; decide
   | .inr (.inr y_gt) => simp [not_lt_of_gt y_gt, ne_of_gt y_gt]; decide
   | .inr (.inl y_eq) =>
     simp [y_eq, lt_irrefl]
     match lt_trichotomy d₁.month d₂.month with
     | .inl m_lt => simp [m_lt]; decide
     | .inr (.inr m_gt) => simp [not_lt_of_gt m_gt, ne_of_gt m_gt]; decide
     | .inr (.inl m_eq) =>
       simp [m_eq, lt_irrefl]
       match lt_trichotomy d₁.day d₂.day with
       | .inl d_lt => simp [d_lt]; decide
       | .inr (.inr d_gt) => simp [not_lt_of_gt d_gt, ne_of_gt d_gt]; decide
       | .inr (.inl d_eq) => simp [d_eq, lt_irrefl d₂.day]; decide
