...
theorem Year.num_days_in_gregorian_year_eq (y : Year) : (y.numDaysInGregorianYear = 365) ∨ (y.numDaysInGregorianYear = 366) := by
  /-
  y : Year
  ⊢ numDaysInGregorianYear y = 365 ∨ numDaysInGregorianYear y = 366
  -/
  by_cases h : y.isLeapYear <;> simp [numDaysInGregorianYear, h, ite_true, add_right_eq_self,
    or_true]

theorem Year.num_days_in_gregorian_year_ge_365 (y : Year) : (365 : Int) <= ↑y.numDaysInGregorianYear := by
  by_cases h : y.isLeapYear <;> simp only [numDaysInGregorianYear, h, ite_true, Int.natCast_add,
    Nat.cast_ofNat, Int.Nat.cast_ofNat_Int] <;> linarith
